...
// Video thumbnail code - Ark. Modified not to show thumbnail for the first gallery image if a video is present (to insert dummy image)
global $product;
...
if ( $attachment_ids && $post_attachment_id ) {
    ...
    foreach ( $attachment_ids as $attachment_id ) {
        // If a video URL is set and this is the first image, skip it
        if ( ! empty( $video_url ) && $loop_index === 0 ) {
            $loop_index++;
            continue;
        }
